...
*************************************************************************/
template < typename MeshType, typename RBFBasisType > class GMMLrbfcmNavierStokes {
private:
    MeshType     MESH;
...
public:
    GMMLrbfcmNavierStokes( MeshType& MMESH, RBFBasisType& RRBFBasis, double vviscous, double ddt,
...
void GMMLrbfcmNavierStokes< MeshType, RBFBasisType >::assembly() {
    ...
    vector< size_t >       neighbours(
        near_num );  // using size_t instead of UINT for compatibility reason with nanoflann.hpp
...
vector< double > GMMLrbfcmNavierStokes< MeshType, RBFBasisType >::CrankNicolsonU(
    ...
    do {
        ...
        cout << "CrankNicolson error at " << IterN << " = " << fixed << setprecision( 10 )
             << curr_error << endl;

        IterN++;

        if ( IterN > CrankNicolsonMaxIter )
            break;
...
template < typename MeshType, typename RBFBasisType >
vector< double > GMMLrbfcmNavierStokes< MeshType, RBFBasisType >::InsideCrankNicolsonU(
    int Istep, const vector< double >& Pn, const vector< double >& Veln,
    const vector< double >& Veltemp ) {
    ...
    for ( int i = 0; i < Nint; i++ ) {
        ...
        RHS[ Nall + i ] =
            2.0 * RHS[ Nall + i ] / viscous - ( 2.0 / dt / viscous ) * Veln[ Nall + i ];
    }
...
template < typename MeshType, typename RBFBasisType >
void GMMLrbfcmNavierStokes< MeshType, RBFBasisType >::SolvePU( int Istep, vector< double >& Pn1,
                                                               vector< double >&       Veln1,
    ...
    gmm::mult_add( IntDxMatrix,
                   gmm::scaled( gmm::sub_vector( Vels, gmm::sub_interval( 0, Nall ) ), 1.0 / dt ),
                   gmm::sub_vector( RHS, gmm::sub_interval( 0, Nint ) ) );
    gmm::mult_add(
        IntDyMatrix,
        gmm::scaled( gmm::sub_vector( Vels, gmm::sub_interval( Nall, Nall ) ), 1.0 / dt ),
        gmm::sub_vector( RHS, gmm::sub_interval( 0, Nint ) ) );
...
template < typename MeshType, typename RBFBasisType >
void GMMLrbfcmNavierStokes< MeshType, RBFBasisType >::DeleteOriginData( string FolderName ) {
